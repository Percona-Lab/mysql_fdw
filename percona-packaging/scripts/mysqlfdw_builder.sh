...
get_sources(){
    ...
    PRODUCT=percona-postgresql-mysql_fdw
    echo "PRODUCT=${PRODUCT}" > mysqlfdw.properties

    PRODUCT_FULL=${PRODUCT}-${VERSION}
    echo "PRODUCT_FULL=${PRODUCT_FULL}" >> mysqlfdw.properties
    echo "VERSION=${PSM_VER}" >> mysqlfdw.properties
    echo "BUILD_NUMBER=${BUILD_NUMBER}" >> mysqlfdw.properties
    echo "BUILD_ID=${BUILD_ID}" >> mysqlfdw.properties
    ...
    echo "REVISION=${REVISION}" >> ${WORKDIR}/mysqlfdw.properties
    rm -rf rpm debian
    cp -r percona-packaging/rpm ./
    cp -r percona-packaging/debian ./
    cd ${WORKDIR}
    #
    source mysqlfdw.properties
    #

    tar --owner=0 --group=0 --exclude=.* -czf ${PRODUCT_FULL}.tar.gz ${PRODUCT_FULL}
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${PSM_BRANCH}/${REVISION}/${BUILD_ID}" >> mysqlfdw.properties
    mkdir $WORKDIR/source_tarball
    mkdir $CURDIR/source_tarball
    cp ${PRODUCT_FULL}.tar.gz $WORKDIR/source_tarball
    cp ${PRODUCT_FULL}.tar.gz $CURDIR/source_tarball
    cd $CURDIR
    rm -rf percona-postgresql-mysql_fdw*
    return
}
...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      INSTALL_LIST="percona-postgresql-common clang-devel llvm-devel percona-postgresql11-devel mariadb-devel git rpm-build rpmdevtools systemd systemd-devel wget libxml2-devel openssl-devel perl perl-libxml-perl perl-DBD-Pg perl-Digest-SHA perl-IO-Socket-SSL perl-JSON-PP zlib-devel gcc make autoconf perl-ExtUtils-Embed"
      yum -y install ${INSTALL_LIST}
...
get_tar(){
    TARBALL=$1
    TARFILE=$(basename $(find $WORKDIR/$TARBALL -name 'percona-postgresql-mysql*.tar.gz' | sort | tail -n1))
    if [ -z $TARFILE ]
    then
        TARFILE=$(basename $(find $CURDIR/$TARBALL -name 'percona-postgresql-mysql*.tar.gz' | sort | tail -n1))
        if [ -z $TARFILE ]
        then
...
get_deb_sources(){
    param=$1
    echo $param
    FILE=$(basename $(find $WORKDIR/source_deb -name "percona-postgresql-mysql*.$param" | sort | tail -n1))
    if [ -z $FILE ]
    then
        FILE=$(basename $(find $CURDIR/source_deb -name "percona-postgresql-mysql*.$param" | sort | tail -n1))
        if [ -z $FILE ]
        then
...
build_srpm(){
    ...
    TARFILE=$(find . -name 'percona-postgresql-mysql*.tar.gz' | sort | tail -n1)
    ...
    rpmbuild -bs --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .generic" \
        --define "version ${VERSION}" rpmbuild/SPECS/mysql_fdw.spec
...
build_rpm(){
    ...
    SRC_RPM=$(basename $(find $WORKDIR/srpm -name 'percona-postgresql-mysql*.src.rpm' | sort | tail -n1))
    if [ -z $SRC_RPM ]
    then
        SRC_RPM=$(basename $(find $CURDIR/srpm -name 'percona-postgresql-mysql*.src.rpm' | sort | tail -n1))
        if [ -z $SRC_RPM ]
        then
...
build_source_deb(){
    ...
    #
    TARFILE=$(basename $(find . -name 'percona-postgresql-mysql*.tar.gz' | sort | tail -n1))
    ...
    mv ${TARFILE} percona-postgresql-mysql-fdw_${VERSION}.orig.tar.gz
    cd ${BUILDDIR}

    cd debian
    rm -rf changelog
    echo "percona-postgresql-mysql-fdw (${VERSION}-${RELEASE}) unstable; urgency=low" >> changelog
    echo "  * Initial Release." >> changelog
    echo " -- EvgeniyPatlan <evgeniy.patlan@percona.com> $(date -R)" >> changelog

    cd ../
    
    dch -D unstable --force-distribution -v "${VERSION}-${RELEASE}" "Update to new percona-postgresql-mysql-fdw version ${VERSION}"
    dpkg-buildpackage -S
    cd ../
...
build_deb(){
    ...
    export ARCH=$(echo $(uname -m) | sed -e 's:i686:i386:g')
    #
    echo "DEBIAN=${DEBIAN}" >> mysqlfdw.properties
    echo "ARCH=${ARCH}" >> mysqlfdw.properties
    ...
    dpkg-source -x ${DSC}
    #
    cd percona-postgresql-mysql-fdw-${VERSION}
...
CURDIR=$(pwd)
VERSION_FILE=$CURDIR/mysqlfdw.properties
args=
WORKDIR=
SRPM=0
SDEB=0
...
REVISION=0
BRANCH="master"
REPO="https://github.com/Percona-Lab/mysqldb_fdw.git"
PRODUCT=percona-postgresql-mysql_fdw
DEBUG=0
